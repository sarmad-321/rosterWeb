...
export default function RosterScheduleTimingDetail() {
  const { code } = useParams();
  const timingCode = code || "";
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "TimingCodeRST") {
  ...
  const fetchDynamicForm = async (paramCode: string) => {
    try {
      setIsLoading(true);
      setLoadError(null);
      ...
      const data = {
        FormName: "RosterScheduleTimings",
        Operation: "read",
        Parameters: { Code: paramCode },
      };
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveRosterScheduleTimingsForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button type="button" onClick={() => fetchDynamicForm(timingCode)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              Retry
            </button>
          </div>
        </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Roster Schedule Timing</h1>
        {timingCode && <p className="text-gray-600">Timing Code: {timingCode}</p>}
      </div>
