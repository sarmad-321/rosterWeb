...
interface TimingCode {
  code: string;
  description: string;
}

export default function RosterScheduleTimings() {
  const navigate = useNavigate();
  const [timingCodes, setTimingCodes] = useState<TimingCode[]>([]);
  ...
  const fetchTimingCodes = async () => {
    try {
      setIsLoading(true);
      setLoadError(null);
      const data = {
        tableDataEnum: dynamicTableEnum.TimingsCode,
        apiParams: "",
      };
      const response = await api.getDynamicTableData(data);
      console.log("Timing codes response:", response);
      if (response?.data && Array.isArray(response.data)) {
        setTimingCodes(response.data);
      }
      setIsLoading(false);
    } catch (error) {
      console.log("Error fetching timing codes:", error);
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load timing codes";
      setLoadError(errorMessage);
      setIsLoading(false);
    }
  };

  const filteredTimings = timingCodes.filter((timing: TimingCode) => {
    const searchLower = searchTerm.toLowerCase();
    return (
      timing.code.toLowerCase().includes(searchLower) ||
      timing.description.toLowerCase().includes(searchLower)
    );
  });

  const handleItemPress = (item: TimingCode) => {
    navigate(`/roster/schedule-timings/${item.code}`);
  };
        ...
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <p className="mt-6 text-lg text-gray-600 font-medium">
              Loading Roster Schedule Timings...
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          Roster Schedule Timings
        </h1>
        <p className="text-gray-600">Manage roster schedule timing codes</p>
      </div>
      ...
      {filteredTimings.length > 0 ? (
              ...
              <div className="flex items-center">
                <div
                  className={classNames(
                    "w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-base mr-3",
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            No Roster Schedule Timings found
          </h3>
          <p className="text-gray-600">
            {searchTerm
              ? "Try adjusting your search criteria"
              : "No timing codes are available"}
          </p>
        </div>
      )}
    </div>
