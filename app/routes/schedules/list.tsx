...
export default function SchedulesList() {
  ...
  useEffect(() => {
    fetchScheduleDefinitionListing();
  }, []);

  const fetchScheduleDefinitionListing = async () => {
    try {
      setIsLoading(true);
      setLoadError(null);
      const data = {
        tableDataEnum: dynamicTableEnum.Schedule,
        apiParams: "",
      };
  ...
  const handleItemPress = (item: Schedule) => {
    navigate(`/schedules/${item.code}`);
  };
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={fetchScheduleDefinitionListing}
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Manage schedule definitions and configurations</p>
      </div>
      ...
      {filteredSchedules.length > 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600">
            {searchTerm ? "Try adjusting your search criteria" : "No schedules are available"}
          </p>
        </div>
      )}
    </div>
