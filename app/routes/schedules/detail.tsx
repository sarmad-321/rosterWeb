...
export default function ScheduleDetail() {
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "ScheduleInput") {
  ...
  const fetchDynamicForm = async (code: string) => {
    try {
      ...
      const data = {
        FormName: "ScheduleDefinitionDetailsBasic",
        Operation: "read",
        Parameters: { ScheduleCode: code },
      };
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveScheduleDefinitionForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button type="button" onClick={() => fetchDynamicForm(scheduleCode)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              Retry
            </button>
          </div>
        </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Schedule Definition Details</h1>
        {scheduleCode && <p className="text-gray-600">Schedule Code: {scheduleCode}</p>}
      </div>
