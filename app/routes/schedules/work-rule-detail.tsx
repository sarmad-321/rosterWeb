...
export default function ScheduleWorkRuleDetail() {
  const { code } = useParams();
  const scheduleRuleCode = code || "";
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "ScheduleWorkRules-Modal") {
  ...
  useEffect(() => {
    fetchDynamicForm(scheduleRuleCode);
    return () => {
  ...
  const fetchDynamicForm = async (code: string) => {
    try {
      setIsLoading(true);
      setLoadError(null);
      ...
      const data = {
        FormName: "ScheduleWorkRuleDefinitionDetails",
        Operation: "read",
        Parameters: { ScheduleRuleCode: code || "" },
      };
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveScheduleWorkRuleDefinitionForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button type="button" onClick={() => fetchDynamicForm(scheduleRuleCode)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              Retry
            </button>
          </div>
        </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Schedule Work Rule Definition Details</h1>
        {scheduleRuleCode && <p className="text-gray-600">Schedule Rule Code: {scheduleRuleCode}</p>}
      </div>
