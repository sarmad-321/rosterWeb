...
export default function ShiftDefinitions() {
  const navigate = useNavigate();
  const [shiftRules, setShiftRules] = useState<ShiftDefinition[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState("");

  useEffect(() => {
    fetchShiftDefinitionListing();
  }, []);

  const fetchShiftDefinitionListing = async () => {
    try {
      setIsLoading(true);
      setLoadError(null);
      const data = {
        tableDataEnum: dynamicTableEnum.Shifts,
        apiParams: "",
      };
      const response = await api.getDynamicTableData(data);
      console.log("Shift rules response:", response);
      if (response?.data && Array.isArray(response.data)) {
        setShiftRules(response.data);
      }
      setIsLoading(false);
    } catch (error) {
      console.log("Error fetching shift definition:", error);
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load shift rules";
      setLoadError(errorMessage);
      setIsLoading(false);
    }
  };

  const filteredRules = shiftRules.filter((rule: ShiftDefinition) => {
  ...
  const handleItemPress = (item: ShiftDefinition) => {
    navigate(`/shifts/definitions/${item.code}`);
  };
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={fetchShiftDefinitionListing}
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Shift Rules</h1>
        <p className="text-gray-600">Manage shift definitions and rules</p>
      </div>
