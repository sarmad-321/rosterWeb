...
export default function ShiftDefinitionDetail() {
  const { code } = useParams();
  const shiftCode = code || "";

  const [currentShiftCode, setCurrentShiftCode] = useState(shiftCode);
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "ShiftCode") {
        fetchDynamicForm(selectedItem?.code);
        setCurrentShiftCode(selectedItem?.code);
      }

      if (currentField?.jquerySelectorID === "HistoryRecordSelect") {
        fetchDynamicForm(currentShiftCode, selectedItem?.pageNo);
      }
    },
  ...
  const fetchDynamicForm = async (code: string) => {
    try {
      ...
      const data = {
        FormName: "ShiftDefinitionDetails",
        Operation: "read",
        Parameters: {
          ShiftCode: code,
        },
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveShiftDefinitionForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={() => fetchDynamicForm(currentShiftCode)}
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        {currentShiftCode && (
          <p className="text-gray-600">Shift Code: {currentShiftCode}</p>
        )}
      </div>
