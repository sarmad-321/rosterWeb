...
export default function ShiftRuleDetail() {
  const { code } = useParams();
  const shiftRuleCode = code || "";
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "ShiftRuleCodeSRDD") {
  ...
  useEffect(() => {
    fetchDynamicForm(shiftRuleCode);
  ...
  const fetchDynamicForm = async (code: string) => {
    try {
      ...
      const data = {
        FormName: "ShiftRuleDefinitionDetails",
        Operation: "read",
        Parameters: {
          ShiftRuleCode: code,
        },
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveShiftRuleDefinitionForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={() => fetchDynamicForm(shiftRuleCode)}
    ...
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          Shift Rule Definition Details
        </h1>
        {shiftRuleCode && (
          <p className="text-gray-600">Shift Rule Code: {shiftRuleCode}</p>
        )}
      </div>
