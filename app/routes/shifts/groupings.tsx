...
interface ShiftGrouping {
  code: string;
  description: string;
}

export default function ShiftGroupings() {
  const navigate = useNavigate();
  const [shiftRules, setShiftRules] = useState<ShiftGrouping[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState("");

  useEffect(() => {
    fetchShiftGroupingDefinitionListing();
  }, []);

  const fetchShiftGroupingDefinitionListing = async () => {
    try {
      setIsLoading(true);
      setLoadError(null);
      const data = {
        tableDataEnum: dynamicTableEnum.ShiftGroup,
        apiParams: "",
      };
      const response = await api.getDynamicTableData(data);
      console.log("Shift groupings response:", response);
      if (response?.data && Array.isArray(response.data)) {
        setShiftRules(response.data);
      }
      setIsLoading(false);
    } catch (error) {
      console.log("Error fetching shift groupings:", error);
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load shift groupings";
      setLoadError(errorMessage);
      setIsLoading(false);
    }
  };

  const filteredRules = shiftRules.filter((rule: ShiftGrouping) => {
  ...
  const handleItemPress = (item: ShiftGrouping) => {
    navigate(`/shifts/groupings/${item.code}`);
  };
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={fetchShiftGroupingDefinitionListing}
    ...
    <div className="p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Manage shift grouping definitions and configurations
        </p>
      </div>
      ...
      {filteredRules.length > 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            No shift groupings found
          </h3>
          <p className="text-gray-600">
            {searchTerm
              ? "Try adjusting your search criteria"
              : "No shift groupings are available"}
          </p>
        </div>
      )}
    </div>
