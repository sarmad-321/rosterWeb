...
export default function ShiftGroupingDetail() {
  const { code } = useParams();
  const shiftGroupingCode = code || "";
  ...
  const customButtonFunctions = {
    afterLoadBtnCick: (selectedItem: any, currentField: any) => {
      ...
      if (currentField?.jquerySelectorID === "ShiftGroup") {
  ...
  useEffect(() => {
    fetchDynamicForm(shiftGroupingCode);
  ...
  const fetchDynamicForm = async (code: string) => {
    try {
      ...
      const data = {
        FormName: "ShiftGroupingDefinitionDetails",
        Operation: "read",
        Parameters: {
          ShiftGroupingCode: code,
        },
      };
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const response = await api.saveShiftGroupingForm(formData);
      ...
      <div className="p-8">
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={() => fetchDynamicForm(shiftGroupingCode)}
    ...
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          Shift Grouping Definition Details
        </h1>
        {shiftGroupingCode && (
          <p className="text-gray-600">Shift Grouping Code: {shiftGroupingCode}</p>
        )}
      </div>
